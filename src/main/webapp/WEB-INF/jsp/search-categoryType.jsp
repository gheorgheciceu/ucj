...
<c:if test="${param.registred eq true}">
	<div class="alert alert-success alertPoz">Categoria a fost
		inregistrata cu succes !</div>
</c:if>

<c:if test="${param.deleted eq true}">
	<div class="alert alert-success alertPoz">Categoria a fost stears
		acu succes !</div>
</c:if>

<c:if test="${param.edited eq true}">
	<div class="alert alert-success alertPoz">Categoria a fost
		editata cu succes !</div>
</c:if>
						...
						<table class="table table-striped table-bordered" id="timetable">
							...
							<tbody>
								<c:forEach items="${categoryType}" var="categoryType">
									<tr>
										<td class="white">${categoryType.type.type}</td>
										<td class="white">${categoryType.type.mandatoryHours}</td>
										<td class="col-md-1"><a
											href="/delete-categoryType/${categoryType.id}"
											data-toggle="tooltip" data-original-title="sterge"
											class="btn btn-danger btn-xs triggerRemove"><i
												class="fa fa-trash-o"></i> </a></td>
										<td class="col-md-1"><a
											href="/edit-categoryType/${categoryType.id}"

...
<c:if test="${param.deleted eq true}">
	<div class="alert alert-success alertPoz">Voluntarul a fost sters
		cu succes !</div>
</c:if>
				...
				<div class="col-sm-12">
					<div class="table-responsive">
						<table class="table table-striped table-bordered" id="timetable">
							...
							<tbody>
								<c:forEach items="${studentType}" var="student">
									<tr>
										<td class="white"><a
											href="<spring:url value="/studentType/${student.id}" />">${student.lastName}</a></td>
										<td class="white">${student.firstName}</td>
										<td class="white">${student.email}</td>
										<%-- <td class="white">${student.category.type.type}</td> --%>
										<%-- 
										<td class="white">${student.phone}</td> --%>
										<td class="col-md-1"><a
											href="<spring:url value="/delete-studentType/${student.id}" />"
											data-toggle="tooltip" data-original-title="dezactiveaz&#259"
											class="btn btn-danger btn-xs triggerRemove"><i
												class="fa fa-trash-o"></i> </a></td>
										<td class="col-md-1"><a
											href="<spring:url value="/edit-studentType/${student.id} " />"
